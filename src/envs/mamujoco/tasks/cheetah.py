...
class HalfCheetahMultiTask(ParallelEnv):
    ...
    def _upright_reward(self) -> float:
        ...
        HalfCheetah 的 ``qpos[2]`` 是躯干绕 y 轴的 pitch 角（弧度），
        奔跑时应接近 0。pitch 偏离越大（翻倒/倒立）奖励越低。
        """
        pitch = float(self._env.single_agent_env.unwrapped.data.qpos[2])
        return float(
            tolerance(
                pitch,
                bounds=(-0.5, 0.5),  # 约 ±28°
    ...
    def _run_reward(
        ...
        return float(speed_reward * self._upright_reward())

    def _run_backwards_reward(
        self,
        x_velocity: float,
    ) -> float:
        """计算 ``run_backwards`` 任务奖励：反向速度高 × 躯干直立。"""
        speed_reward = tolerance(
            -x_velocity,
    ...
    def _jump_reward(
        ...
        """
        计算 ``jump`` 任务奖励：双脚同时离地 + 躯干水平 + 接近原地。

        用 ``min(ffoot_z, bfoot_z)`` 代替平均值，避免倒立把单脚举起来
        就拿满分；再乘以躯干直立项，禁止头朝下蹦。
        """
        ffoot_z = self._get_body_z("ffoot")
        bfoot_z = self._get_body_z("bfoot")
        min_foot = min(ffoot_z, bfoot_z)
        ...
        return float(upright * (5 * height_reward + speed_reward) / 6)
